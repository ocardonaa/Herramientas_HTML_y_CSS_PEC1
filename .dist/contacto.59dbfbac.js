...
function checkEmail(input) {
    const isEmail = /^.+@.+$/;
    if (isEmail.test(input.value.trim())) showSuccess(input);
    else showError(input, `${getFieldName(input)} es obligatorio`);
}
function checkRequired(inputArr) {
    inputArr.forEach((element)=>{
        if (element.value.trim() === "") showError(element, `${getFieldName(element)} es obligatorio`);
...
function checkAllGreen(inputArr) {
    let allGreens = true;
    inputArr.forEach((element)=>{
        if (element.style.borderColor !== "green") allGreens = false;
    });
    return allGreens;
}
...
//# sourceMappingURL=contacto.59dbfbac.js.map
